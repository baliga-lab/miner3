...
def fileToReferenceDictionary(filename,dictionaryName):
...
def tfbsdbEnrichment(task,p=0.05):
    ...
    start, stop = task[0]
    allGenes,revisedClusters,tfMap,tfToGenes = task[1]
...
def mechanisticInference(axes,revisedClusters,expressionData,correlationThreshold=0.3,numCores=5,database="tfbsdb_tf_to_genes.pkl",dataFolder=os.path.join(os.path.split(os.getcwd())[0],"data")):
    import os
    ...
    tasks = [[taskSplit[i],(allGenes,revisedClusters,tfMap,tfToGenes)] for i in range(len(taskSplit))]
