...
def orderMembership(centroidMatrix,membershipMatrix,mappedClusters,ylabel="",resultsDirectory=None):
    ...
    asp = ordered_matrix.shape[1]/ordered_matrix.shape[0]
    ax.set_aspect(asp)
    ax.grid(False)
...
def plotDifferentialMatrix(overExpressedMembersMatrix,underExpressedMembersMatrix,orderedOverExpressedMembers,saveFile=None):
    ...
    ax.imshow(orderedDM,cmap='viridis',vmin=-1,vmax=1)
    asp = differentialActivationMatrix.shape[1]/differentialActivationMatrix.shape[0]
    ax.set_aspect(asp)
    ax.grid(False)
