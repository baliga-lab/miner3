...
gene_aucs_maf_MMRF = miner.gene_aucs(np.array(x),y)
aucs_all_MMRF, args_all_MMRF = miner.gene_aucs(np.array(x),y,return_aucs=True)
genes_all_MMRF = x.index[args_all_MMRF]
...
plt.savefig(os.path.join(resultsDirectory,"ROC_regulons"),bbox_inches="tight")
